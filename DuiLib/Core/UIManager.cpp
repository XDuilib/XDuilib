...
int CPaintManagerUI::GetOptionGroupIndex(LPCTSTR pStrGroupName)
{
	int Index = -1;
	LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
	if( lp ){
		CStdPtrArray* aOptionGroup = static_cast<CStdPtrArray*>(lp);
		for (Index = 0; Index < aOptionGroup->GetSize(); Index++)
		{
			if (static_cast<COptionUI*>(aOptionGroup->GetAt(Index))->IsSelected())
			{
				break;
			}
		}
